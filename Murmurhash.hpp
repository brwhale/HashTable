...
template <typename T>
inline uint64_t hash_fast(T& item) {
	return MurmurHash((void*)&item, static_cast<int>(sizeof(T)));
}

template <>
inline uint64_t hash_fast<std::string>(std::string& item) {
	return MurmurHash((void*)item.data(), static_cast<int>(item.size()));
}
